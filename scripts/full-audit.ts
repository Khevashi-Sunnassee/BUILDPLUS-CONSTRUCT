...
function classifyPageType(routePath: string, pageTitle: string, code: string): string {
...
function analyzePageCode(filePath: string, routePath: string, pageTitle: string, module: string): AuditScores {
  ...
  const isDetail = /:id|:jobId|:date/.test(routePath) || /Detail|Form|Edit|Fill/.test(pageTitle);
...
async function runFullAudit() {
  ...
  for (const row of targets.rows) {
    ...
    try {
      ...
      console.log(`  [${audited}/${targets.rows.length}] ${target.pageTitle} (${target.routePath}) -> ${overallScore}/5 [${dims.join(",")}]`);
    } catch (err: any) {
      errors++;
  ...
  for (const s of scoreSummary) dist[s.score - 1]++;
  ...
  console.log(`Distribution: 1★=${dist[0]} | 2★=${dist[1]} | 3★=${dist[2]} | 4★=${dist[3]} | 5★=${dist[4]}`);

  process.exit(0);
}

function generateFindingsSummary(pageTitle: string, routePath: string, module: string, scores: AuditScores): string {
  const dims = [
