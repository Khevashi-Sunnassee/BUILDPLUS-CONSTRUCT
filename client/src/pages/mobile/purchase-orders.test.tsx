...
vi.mock("wouter", () => ({
  useLocation: () => ["/mobile/purchase-orders", vi.fn()],
...
import MobilePurchaseOrdersPage from "./purchase-orders";

describe("MobilePurchaseOrdersPage", () => {
  beforeEach(() => {
    mockUseQuery.mockReturnValue({ data: [], isLoading: false });
  });

  it("renders the purchase orders page", () => {
    renderWithProviders(<MobilePurchaseOrdersPage />);
    expect(screen.getByTestId("mock-mobile-nav")).toBeInTheDocument();
  });

  it("shows loading state when data is loading", () => {
    mockUseQuery.mockReturnValue({ data: undefined, isLoading: true });
    const { container } = renderWithProviders(<MobilePurchaseOrdersPage />);
    expect(container.firstChild).toBeInTheDocument();
  });

  it("displays purchase orders heading", () => {
    renderWithProviders(<MobilePurchaseOrdersPage />);
