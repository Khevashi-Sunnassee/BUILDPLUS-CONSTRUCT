...
vi.mock("wouter", () => ({
  useLocation: () => ["/mobile/broadcast", vi.fn()],
...
import MobileBroadcastPage from "./broadcast";

describe("MobileBroadcastPage", () => {
  beforeEach(() => {
    mockUseQuery.mockReturnValue({ data: [], isLoading: false });
  });

  it("renders the broadcast page", () => {
    renderWithProviders(<MobileBroadcastPage />);
    expect(screen.getByTestId("mock-mobile-nav")).toBeInTheDocument();
  });

  it("shows loading state when data is loading", () => {
    mockUseQuery.mockReturnValue({ data: undefined, isLoading: true });
    const { container } = renderWithProviders(<MobileBroadcastPage />);
    expect(container.firstChild).toBeInTheDocument();
  });

  it("displays broadcast heading", () => {
    renderWithProviders(<MobileBroadcastPage />);
