...
vi.mock("wouter", () => ({
  useLocation: () => ["/mobile/logistics/delivery", vi.fn()],
...
import MobileRecordDeliveryPage from "./record-delivery";

describe("MobileRecordDeliveryPage", () => {
  beforeEach(() => {
    mockUseQuery.mockReturnValue({ data: [], isLoading: false });
  });

  it("renders the record delivery page", () => {
    renderWithProviders(<MobileRecordDeliveryPage />);
    expect(screen.getByTestId("mock-mobile-nav")).toBeInTheDocument();
  });

  it("shows loading state when data is loading", () => {
    mockUseQuery.mockReturnValue({ data: undefined, isLoading: true });
    const { container } = renderWithProviders(<MobileRecordDeliveryPage />);
    expect(container.firstChild).toBeInTheDocument();
  });

  it("displays record delivery heading", () => {
    renderWithProviders(<MobileRecordDeliveryPage />);
