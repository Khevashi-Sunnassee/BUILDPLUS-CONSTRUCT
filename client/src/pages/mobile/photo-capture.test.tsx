...
vi.mock("wouter", () => ({
  useLocation: () => ["/mobile/photo-capture", vi.fn()],
...
import MobilePhotoCaptue from "./photo-capture";

describe("MobilePhotoCaptue", () => {
  beforeEach(() => {
    mockUseQuery.mockReturnValue({ data: [], isLoading: false });
  });

  it("renders the photo capture page", () => {
    renderWithProviders(<MobilePhotoCaptue />);
    expect(screen.getByTestId("mock-mobile-nav")).toBeInTheDocument();
  });

  it("shows loading state when data is loading", () => {
    mockUseQuery.mockReturnValue({ data: undefined, isLoading: true });
    const { container } = renderWithProviders(<MobilePhotoCaptue />);
    expect(container.firstChild).toBeInTheDocument();
  });

  it("displays photo capture heading", () => {
    renderWithProviders(<MobilePhotoCaptue />);
    expect(screen.getByTestId("text-page-title")).toHaveTextContent("Add Photo");
  });
