...
vi.mock("wouter", () => ({
  useLocation: () => ["/mobile/logistics/return", vi.fn()],
...
import MobileReturnLoadPage from "./return-load";

describe("MobileReturnLoadPage", () => {
  beforeEach(() => {
    mockUseQuery.mockReturnValue({ data: [], isLoading: false });
  });

  it("renders the return load page", () => {
    renderWithProviders(<MobileReturnLoadPage />);
    expect(screen.getByTestId("mock-mobile-nav")).toBeInTheDocument();
  });

  it("shows loading state when data is loading", () => {
    mockUseQuery.mockReturnValue({ data: undefined, isLoading: true });
    const { container } = renderWithProviders(<MobileReturnLoadPage />);
    expect(container.firstChild).toBeInTheDocument();
  });

  it("displays return load heading", () => {
    renderWithProviders(<MobileReturnLoadPage />);
