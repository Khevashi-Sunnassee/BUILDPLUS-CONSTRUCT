...
vi.mock("wouter", () => ({
  useLocation: () => ["/mobile/checklists", vi.fn()],
...
import MobileChecklistsPage from "./checklists";

describe("MobileChecklistsPage", () => {
  beforeEach(() => {
    mockUseQuery.mockReturnValue({ data: [], isLoading: false });
  });

  it("renders the checklists page", () => {
    renderWithProviders(<MobileChecklistsPage />);
    expect(screen.getByTestId("mock-mobile-nav")).toBeInTheDocument();
  });

  it("shows loading state when data is loading", () => {
    mockUseQuery.mockReturnValue({ data: undefined, isLoading: true });
    const { container } = renderWithProviders(<MobileChecklistsPage />);
    expect(container.firstChild).toBeInTheDocument();
  });

  it("displays checklists heading", () => {
    renderWithProviders(<MobileChecklistsPage />);
