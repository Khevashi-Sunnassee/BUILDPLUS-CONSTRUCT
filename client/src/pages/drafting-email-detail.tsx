      ...
      <CardContent className="p-4 space-y-3">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          ...
          {(dismissed || !hasAiRecommendation) && !showManual && (
            <Button size="sm" variant="outline" onClick={() => { setShowManual(true); setDismissed(false); }} data-testid="button-open-create-task">
        ...
        {hasAiRecommendation && !dismissed && !showManual && (
          <div className="space-y-3" data-testid="panel-ai-recommendation">
            <div className="rounded-md border bg-indigo-50/50 dark:bg-indigo-950/20 border-indigo-200 dark:border-indigo-800 p-3 space-y-2">
              <p className="text-sm font-medium" data-testid="text-ai-task-title">{aiTitle}</p>
              {aiDescription && (
                <p className="text-xs text-muted-foreground leading-relaxed" data-testid="text-ai-task-description">{aiDescription}</p>
              )}
              <div className="flex items-center gap-2 flex-wrap">
                {aiActionType && (
                  <Badge variant="outline" className="text-xs" data-testid="badge-ai-action-type">{aiActionType}</Badge>
                )}
                <Badge className={`text-xs ${priorityColors[aiPriority]}`} data-testid="badge-ai-priority">{aiPriority}</Badge>
                {aiDueDate && (
                  <span className="text-xs text-muted-foreground" data-testid="text-ai-due-date">
                    Due: {formatDate(aiDueDate)}
                  </span>
                )}
              ...
              {aiReason && (
                <p className="text-xs text-muted-foreground italic" data-testid="text-ai-reason">{aiReason}</p>
              )}
              {matchedJobLabel && (
                <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                  <LinkIcon className="h-3 w-3 shrink-0" />
                  <span>{matchedJobLabel}</span>
                </div>
              )}
            </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Button
                ...
                data-testid="button-ignore-task"
              >
                Ignore
              </Button>
              <Button
        ...
        {!hasAiRecommendation && !showManual && !dismissed && (
          <div className="text-center py-3">
            <p className="text-sm text-muted-foreground mb-2">No AI recommendation available for this email.</p>
            <Button size="sm" variant="outline" onClick={() => setShowManual(true)} data-testid="button-open-create-task">
              <Pencil className="h-3 w-3 mr-1" />
        ...
        {showManual && (
          <div className="space-y-3 pt-1" data-testid="panel-manual-task">
            {matchedJobLabel && (
            ...
            <div data-testid="field-task-assignee">
              ...
              <Select value={manualAssigneeId} onValueChange={setManualAssigneeId}>
                <SelectTrigger data-testid="select-task-assignee" className={!manualAssigneeId ? "border-destructive/50" : ""}>
                  <SelectValue placeholder="Select a user..." />
                </SelectTrigger>
                <SelectContent>
                  {companyUsers.map(user => (
