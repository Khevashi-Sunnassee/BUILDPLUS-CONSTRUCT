...
- **Pre-Sales Opportunity Management**: Mobile-first opportunity entry form (`/mobile/opportunities/new`) for field sales team with customer select/quick-add, location, people, financials & scale, status & likelihood (probability bar), timeline, and notes sections. Desktop Sales Pipeline page (`/sales-pipeline`) with summary cards (Total Pipeline, Weighted Value, Active Opportunities, Avg Probability), filterable table, and detail dialog with inline status updates. Jobs table extended with `referrer`, `engineerOnJob`, `estimatedValue`, `numberOfLevels`, `opportunityStatus` (enum: NEW, CONTACTED, PROPOSAL_SENT, NEGOTIATING, WON, LOST, ON_HOLD), `probability`, `estimatedStartDate`, `comments`. Job status enum expanded with OPPORTUNITY, QUOTING, WON, LOST, CANCELLED, CONTRACTED, IN_PROGRESS. Auto-generates `OPP-XXX` job numbers. API: `GET/POST /api/jobs/opportunities`, `PATCH /api/jobs/opportunities/:id`, `POST /api/customers/quick`. Company-scoped with customer ownership validation.
