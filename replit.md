...
- **Document Management System**: Comprehensive document register with file upload/download via Replit Object Storage, version control (parent_document_id chain, isLatestVersion flag), document bundles with QR code access for secure guest sharing (with expiration, access logging, IP tracking), entity linking (jobs, panels, suppliers, purchase orders, tasks), configurable document types/disciplines/categories, and pagination support for 1000+ documents.
