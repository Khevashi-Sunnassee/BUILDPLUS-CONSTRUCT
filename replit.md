...
- **Pre-Sales Opportunity Management**: Mobile-first opportunity entry form (`/mobile/opportunities/new`) for field sales team with customer select/quick-add, location, people, financials & scale, sales pipeline stage/status, opportunity type, probability bar, timeline, and notes sections. Desktop Sales Pipeline page (`/sales-pipeline`) with summary cards (Total Pipeline, Weighted Value, Active Opportunities, Avg Probability), stage pipeline filter chips, filterable table with sales stage/status/type columns, and detail dialog with tabbed UI (Details/History) for inline stage/status updates with optional notes and full audit history. Advanced sales pipeline system with 6 stages (OPPORTUNITY, PRE_QUALIFICATION, ESTIMATING, SUBMITTED, AWARDED, LOST), each with contextual statuses defined in `shared/sales-pipeline.ts`. Opportunity type classification (Builder Selected, Open Tender, Negotiated Contract, General Pricing), primary contact tracking, and `sales_status_history` table for audit trail. Jobs table extended with `salesStage`, `salesStatus`, `opportunityType`, `primaryContact`, plus legacy fields `referrer`, `engineerOnJob`, `estimatedValue`, `numberOfLevels`, `opportunityStatus`, `probability`, `estimatedStartDate`, `comments`. Job status enum expanded with OPPORTUNITY, QUOTING, WON, LOST, CANCELLED, CONTRACTED, IN_PROGRESS. Auto-generates `OPP-XXX` job numbers. API: `GET/POST /api/jobs/opportunities`, `PATCH /api/jobs/opportunities/:id`, `GET /api/jobs/opportunities/:id/history`, `POST /api/customers/quick`. Server validates stage/status pairing via `isValidStatusForStage()`. Company-scoped with customer ownership validation.
...
- **Mobile Panel Checklists**: System-locked checklist modules (Panels, Documents) with `isSystem` flag on entity_types and checklist_templates tables. Default Pre-Pour and Post-Pour Quality Inspection templates auto-seeded on startup via `ensureSystemChecklistModules()`. Pre-Pour covers formwork, reinforcement, embeds, defect assessment; Post-Pour covers surface finish, dimensional check, lifting points, defect assessment. Templates use `dependsOn`/`dependsOnValue` for conditional field visibility. System modules and templates cannot be deleted (403 response). Mobile panel detail page has tabbed UI (Details/Inspections/History) with ability to start, resume, and complete inspections. Checklist fill page navigates back to panel detail when instance has panelId.
