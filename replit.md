...
- **Budget System:** Comprehensive 4-phase cost management system:
  - *Cost Codes:* Company-level cost code library (`cost_codes`) with hierarchical parent/child support, admin management page at `/admin/cost-codes`. Cost code defaults per job type (`cost_code_defaults`) with checkbox-based assignment. Per-job cost codes (`job_cost_codes`) inherited from job type defaults with customization. RBAC key: `admin_cost_codes`.
  - *Tender Center:* Full tender lifecycle management at `/tenders`. Tenders per job with status workflow (DRAFT → OPEN → UNDER_REVIEW → APPROVED → CLOSED → CANCELLED). Tender submissions from suppliers with line items linked to cost codes. Submission approval workflow. Auto-generated tender numbers (TDR-000001). Schema: `tenders`, `tender_packages`, `tender_submissions`, `tender_line_items`, `tender_line_activities`, `tender_line_files`, `tender_line_risks`. RBAC key: `tenders`.
  - *Job Budget:* Per-job budget management at `/jobs/:id/budget`. Top-level budget with estimated total, profit target %, customer price. Budget lines per cost code linking to selected tender submissions and contractors. Variation tracking and forecast costs. Summary calculations. Schema: `job_budgets`, `budget_lines`, `budget_line_files`. RBAC key: `budgets`.
  - *Bill of Quantities (BOQ):* Detailed item-level costing at `/jobs/:id/boq`. BOQ groups (buildings/levels) under cost codes. BOQ items with quantities, units (EA/SQM/M3/LM/M2/M/HR/DAY/TONNE/KG/LOT), unit pricing, line totals. Links to tender line items. Schema: `boq_groups`, `boq_items`. RBAC key: `budgets`.
