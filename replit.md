...
- **Mobile Functionality:** QR scanner, mobile panel checklists with conditional fields, mobile PM Call Logs, and mobile Work Orders with pull-to-refresh, error recovery with retry, client-side pagination, quick status actions (Start Work / Mark Resolved), and full asset/cost/description detail views. Offline-first PWA capability with IndexedDB (idb) for Tasks, Checklists, and Photos — service worker (`client/public/sw.js`) caches app shell and API responses; outbox queue pattern stores mutations when offline and syncs sequentially on reconnect; `useOfflineQuery` and `useOfflineMutation` hooks in `client/src/lib/offline/` wrap existing React Query patterns; server-side `processed_sync_actions` table provides idempotency via UUID action dedup; `OfflineStatusBadge` component shows connection/sync state on mobile pages; manifest.json enables PWA install.
- **Dual Logo System:** Two-tier branding with System Logo (managed by Super Admin, displayed in sidebar/login/notifications) and User Logo (managed by company admins in Company Branding, used on all printed documents/PDFs/outbound communications). `global_settings.logoBase64` stores the system logo; `global_settings.userLogoBase64` stores the user logo. Print/PDF outputs use `userLogoBase64 || logoBase64` fallback pattern. System logo endpoint (`POST /api/admin/settings/logo`) requires Super Admin; user logo endpoint (`POST /api/admin/settings/user-logo`) requires company ADMIN role.
...
- **Email Dispatch:** Enterprise async queue-based email dispatch with database-backed persistence (`email_queue_jobs` table), dead letter queue (`email_dead_letters` table) for permanently failed emails, token bucket rate limiting, per-company UTC-based daily quotas, cache pruning, exponential backoff retry, and automatic recovery of stale jobs on server restart.
