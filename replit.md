...
## Recent Changes
- **Feb 2026:** Company Email Inboxes: Added company_email_inboxes table (DRAFTING, TENDER, AP_INVOICES, GENERAL types) with replyToAddress, isDefault per type, email uniqueness. CRUD routes with company isolation and ADMIN role. Mail register routes accept fromInboxId, validate against company + active status, pass custom from/replyTo through dispatch queue to Resend. Email service supports custom from address with display name formatting. Frontend: OutgoingInboxesCard in Settings > Email tab with add/edit/delete dialogs. EmailComposeDialog auto-selects default inbox and shows "From Inbox" selector for active inboxes.
