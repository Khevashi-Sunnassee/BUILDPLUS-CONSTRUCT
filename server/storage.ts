...
import {
  ...
  broadcastTemplates, broadcastMessages, broadcastDeliveries,
...
export class DatabaseStorage implements IStorage {
  ...
  // Documents - with pagination support for 1000+ documents
  ...
  async getNextDocumentNumber(typeId: string): Promise<string> {
    ...
    // Count existing documents with this prefix for this year
    const [countResult] = await db.select({ count: sql<number>`count(*)` })
      .from(documents)
  ...
  async getDocumentVersionHistory(documentId: string): Promise<Document[]> {
    // First get the document to find its root
    const [doc] = await db.select().from(documents).where(eq(documents.id, documentId));
    if (!doc) return [];

    // Find the root document (one with no parent)
  ...
  async createNewVersion(parentDocumentId: string, data: InsertDocument): Promise<Document> {
    // Mark previous version as not latest and superseded
  ...
  async getDocumentBundleByQr(qrCodeId: string): Promise<DocumentBundleWithItems | undefined> {
    ...
    if (!bundle) return undefined;
    return this.getDocumentBundle(bundle.id);
  }
