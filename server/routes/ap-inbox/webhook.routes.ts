...
import { db, logger, objectStorageService, logActivity, getResendApiKey } from "./shared";
import {
  apInboundEmails, apInboxSettings, apInvoices, apInvoiceDocuments,
  apInvoiceSplits, companies, suppliers,
  tenderInboxSettings, draftingInboxSettings
} from "@shared/schema";
import { handleTenderInboundEmail } from "./tender-processing";
import { handleDraftingInboundEmail } from "./drafting-processing";
    ...
    try {
      ...
      const [allApSettings, allTenderSettings, allDraftingSettings] = await Promise.all([
        db.select({ settings: apInboxSettings, companyName: companies.name })
          ...
          .limit(1000),
        db.select({ settings: tenderInboxSettings, companyName: companies.name })
          .from(tenderInboxSettings)
      ...
      if (!matchedApSettings) {
        for (const { settings } of allTenderSettings) {
          if (settings.inboundEmailAddress) {
            const normalizedInbound = settings.inboundEmailAddress.toLowerCase().trim();
            if (toAddresses.some(addr => addr.toLowerCase().trim() === normalizedInbound)) {
              matchedTenderSettings = settings;
              break;
            }
          }
        }
      ...
      const [inboundRecord] = await db.insert(apInboundEmails).values({
        companyId: matchedApSettings.companyId,
